...
export const Route = createFileRoute("/journal")({
  head: () => ({
    meta: [
      { title: "Trade Journal — DayTrader Pro" },
      { name: "description", content: "Complete trade history journal with filtering and export." },
    ],
  }),
  component: Journal,
});
...
function Journal() {
  ...
  function exportCsv() {
    const headers = ["id","date","time","asset","side","strategy","session","qty","entry","exit","rr","pnl"];
    const rows = trades.map((t) => [t.id, t.date, t.time, t.asset, t.side, t.strategy, t.session, t.qty, t.entry, t.exit, t.rr, t.pnl].join(","));
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-sm">
              <thead>
                <tr className="border-b border-border text-left text-xs uppercase tracking-wide text-muted-foreground">
                  {["Date", "Time", "Asset", "Side", "Strategy", "Session", "Qty", "Entry", "Exit", "R:R", "P&L", ""].map(
                    (h) => (
              ...
              <tbody>
                {trades.slice(0, 100).map((t) => (
                  <tr key={t.id} className="border-b border-border/60 last:border-0 hover:bg-accent/40">
                    ...
                    <td className="px-3 py-2">{t.rr.toFixed(2)}</td>
                    <td
            ...
            {!isLoading && trades.length === 0 && (
              <div className="py-10 text-center text-sm text-muted-foreground">
                Nenhum trade encontrado. <Link to="/entry" className="font-medium text-primary hover:underline">Registrar primeiro trade</Link>.
              </div>
            )}
          </div>
          {trades.length > 100 && (
            <div className="mt-3 text-center text-xs text-muted-foreground">
              Mostrando 100 de {trades.length} trades
            </div>
          )}
        </Panel>
      </div>
